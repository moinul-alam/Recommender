...
class ModelTraining:
    ...
    def _create_data_generator(self, matrix: sparse.csr_matrix) -> Iterator[Tuple[np.ndarray, np.ndarray, np.ndarray]]:
        ...
        # Yield batches
        for start_idx in range(0, len(ratings), self.batch_size):
            end_idx = min(start_idx + self.batch_size, len(ratings))
    ...
    def _process_batch(self, user_batch, item_batch, rating_batch):
        """Process a single batch and return the batch loss with numeric stability."""
        ...
        # Compute predictions with numeric stability
        dot_products = np.zeros(len(user_batch), dtype=np.float32)
        for f in range(self.n_factors):
            product = user_factors_batch[:, f] * item_factors_batch[:, f]
            # Clip to prevent overflow in summation
            product = np.clip(product, -1e3, 1e3)
            dot_products += product
        ...
        # Compute updates with gradient clipping
        lr = self.learning_rate
        reg = self.regularization
        clip_threshold = 10.0
        
        # Compute gradient updates
        user_factors_update = lr * (errors[:, np.newaxis] * item_factors_batch - reg * user_factors_batch)
        item_factors_update = lr * (errors[:, np.newaxis] * user_factors_batch - reg * item_factors_batch)
        
        # Clip gradients to prevent overflow
        user_factors_update = np.clip(user_factors_update, -clip_threshold, clip_threshold)
        item_factors_update = np.clip(item_factors_update, -clip_threshold, clip_threshold)
        
        # Aggregate updates for unique users and items
        user_updates = np.zeros_like(user_factors_all, dtype=np.float32)
        item_updates = np.zeros_like(item_factors_all, dtype=np.float32)
        
        # Use for loop instead of np.add.at to have more control
        for i, (ui, uf) in enumerate(zip(user_inv, user_factors_update)):
            user_updates[ui] += uf
        
        for i, (ii, iuf) in enumerate(zip(item_inv, item_factors_update)):
            item_updates[ii] += iuf
        
        # Clip aggregated updates
        user_updates = np.clip(user_updates, -clip_threshold, clip_threshold)
        item_updates = np.clip(item_updates, -clip_threshold, clip_threshold)
        
        # Apply updates to HDF5 datasets
        ...
        self.param_file['user_factors'][unique_users] = np.clip(curr_user_factors + user_updates, -100, 100)
        self.param_file['item_factors'][unique_items] = np.clip(curr_item_factors + item_updates, -100, 100)
        
        # Update biases with clipping
        user_bias_updates = lr * (errors - reg * self.user_biases[user_batch])
        user_bias_updates = np.clip(user_bias_updates, -1.0, 1.0)
        self.user_biases[user_batch] += user_bias_updates
        
        item_bias_updates = lr * (errors - reg * self.item_biases[item_batch])
        item_bias_updates = np.clip(item_bias_updates, -1.0, 1.0)
        self.item_biases[item_batch] += item_bias_updates
        
        # Make sure biases stay within reasonable range
        self.user_biases = np.clip(self.user_biases, -5, 5)
        self.item_biases = np.clip(self.item_biases, -5, 5)
        
        # Return mean squared error, handling potential NaN
        mse = np.mean(np.square(errors))
        if np.isnan(mse) or np.isinf(mse):
            self.logger.warning("NaN or inf encountered in loss calculation, using fallback value")
    ...
    def _compute_validation_loss(self, val_matrix: sparse.csr_matrix) -> float:
        """Compute validation loss efficiently using batched processing."""
        total_loss = 0
        n_batches = 0
