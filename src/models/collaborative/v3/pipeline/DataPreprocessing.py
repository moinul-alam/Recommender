...
class DataPreprocessing:
    ...
    def create_user_item_matrix(self, df: pd.DataFrame) -> sparse.csr_matrix:
        ...
        for start in range(0, len(df), self.chunk_size):
            ...
            chunk_matrix = sparse.coo_matrix(
                (chunk["rating"].values, (chunk["user_id"].values, chunk["tmdb_id"].values)),
        ...
        user_means = np.array(user_item_matrix.sum(axis=1)).flatten() / (np.maximum(user_item_matrix.getnnz(axis=1), 1))

        user_item_matrix = user_item_matrix.copy()

        logger.info("Mean-centering user-item matrix...")
        ...
        if self.normalization:
            logger.info(f"Normalizing user-item matrix using {self.normalization} normalization...")
    ...
    def process(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, dict, dict, dict, dict, sparse.csr_matrix]:
        ...
        df, user_mapping, user_reverse_mapping, item_mapping, item_reverse_mapping = self.create_mappings(df)
