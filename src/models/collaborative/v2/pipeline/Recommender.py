...
class Recommender:
    ...
    def generate_recommendations(
        ...
        try:
            ...
            # Compute user preference vector using weighted average
            item_vectors = self.item_matrix[list(item_indices)]
            user_vector = np.average(item_vectors, axis=0, weights=item_ratings)

            # Search for similar items, excluding input items
            search_size = min(2 * n_recommendations, self.item_matrix.shape[0])
            D, I = self.faiss_index.search(user_vector.reshape(1, -1), search_size)
            ...
            for idx, dist in zip(I[0], D[0]):
                if idx >= 0 and idx not in query_items_set and dist >= self.min_similarity:
                    ...
                    similarity_score = max(0, min(100, (1 - (dist / max_distance)) * 100))
                    ...
                    recommendations.append({
                        "tmdb_id": int(tmdb_id),  # Ensure tmdb_id is JSON-serializable
                        "similarity": float(similarity_score),  # Convert to Python float
                        "predicted_rating": float(round(predicted_rating, 2))  # Convert to Python float
                    })
