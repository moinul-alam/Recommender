...
class FeatureEngineering:
    def __init__(
        ...
        self.onehot_genres = OneHotEncoder(sparse_output=True, handle_unknown='ignore')
    ...
    def fit_transformers(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            # Fit genres with one-hot encoding
            logger.info('Fitting OneHotEncoder on genres')
            # Prepare genres data for one-hot encoding
            genres_list = df['genres'].str.split(',').tolist()
            # Convert list of lists to DataFrame for one-hot encoding
            genres_df = pd.DataFrame(genres_list)
            genres_matrix = self.onehot_genres.fit_transform(genres_df)
            ...
            # Clear memory
            del cast_features, director_features
            gc.collect()
    ...
    def transform_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Transform genres with one-hot encoding
            genres_list = df['genres'].str.split(',').tolist()
            genres_df = pd.DataFrame(genres_list)
            genres_matrix = self.onehot_genres.transform(genres_df)
            genres_matrix = normalize(genres_matrix, norm='l2', axis=1)
            
            # Clear memory
            del genres_df, genres_list
            gc.collect()
    ...
    def save_transformers(self, path: str) -> None:
        ...
        transformers = {
            'onehot_genres': self.onehot_genres,
    ...
    def load_transformers(self, path: str) -> None:
        ...
        try:
            ...
            self.onehot_genres = joblib.load(path / "onehot_genres.pkl")
