...
class FeatureEngineering:
    def __init__(
        ...
        self._initialize_transformers(n_components_pca)
        
    def _initialize_transformers(self, n_components_pca: int):
    ...
    def fit_transformers(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            logger.info("Fitting TF-IDF for overview")
            ...
            overview_reduced = overview_matrix
            ...
            logger.info("Fitting FeatureHasher for cast and director")
            ...
            self.metrics['peak_memory'] = max(self._get_memory_usage(), initial_memory)
    ...
    def transform_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            logger.info(f"Transforming features for DataFrame with shape: {df.shape}")
            ...
            genres_matrix = self.mlb_genres.transform(df['genres'].fillna('').apply(lambda x: x.split(',')))
            cast_matrix = self.cast_hasher.transform(df['cast'].fillna('').apply(self._process_cast))
            director_matrix = self.director_hasher.transform(df['director'].fillna('').apply(self._process_directors))
            ...
            combined_features = normalize(combined_features, norm='l2', axis=1)
            transformed_features = self.pca.transform(combined_features.toarray())

            # Create DataFrame with tmdb_id
